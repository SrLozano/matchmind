...
class PaymentsAsyncTest(unittest.IsolatedAsyncioTestCase):
    async def test_checkout_session_creation_uses_standard_price_without_referral(self) -> None:
        ...
        self.assertEqual(created_kwargs["automatic_tax"], {"enabled": True})
        self.assertNotIn("billing_address_collection", created_kwargs)
        self.assertEqual(created_kwargs["customer_creation"], "always")
